...
when not defined(js):
  ...
  proc toJsonHook*(ps: PerformanceSetting): JsonNode =
    result = newJObject()
    result["kind"] = newJString($ps.kind)
    result["fileCheckThrottlingMs"] = newJInt(ps.fileCheckThrottling.inMilliseconds)
    result["updateOnChange"] = newJBool(ps.updateOnChange)
    result["description"] = newJString(ps.description)
...
func messageTypToStr*(typ: MessageType): cstring =
  ...
  of MessageType.Info, MessageType.Log, MessageType.Debug: "info"
